...
//	//the node we will eventually insert
//	Node<T>* ins = new Node<T>();
//	ins->data = data;
//
//	//the node which will be used to iterate across the list
//	Node<T>* temp = nullptr;
//	
//	//get temp to the fourth node of the list
//	temp = first;
//	for (int i = 0; i < 4; i++) {
//		temp = temp->link;
//	}
//
//	//set the new element's link to the current fifth node, which is where temp is currently pointing
//	ins->link = temp->link;
//
//	//set temp(the fourth element)'s link to ins, which makes ins the fifth element
//	temp->link = ins;
//}
//
//template <typename T>
...
//	}
//
//	//temporarily store a pointer to the fifth element, so we can delete it once the references are in order
//	Node<T>* rem = temp->link;
//
//	//skip over the fifth element, removing it from the chain of references
//	temp->link = rem->link;
//
//	delete rem;
//}
